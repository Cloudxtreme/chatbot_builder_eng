...
def answer_stat_importer(user, project):
    fw1 = open('./a_builder/importer/data/answer_stat_' + user + '_' + project, 'r', encoding='utf8')
    lines = fw1.readlines()
    for line in lines:
        line = line.replace('\n', '')
        arr = line.split("^")
        for i in range(len(arr)):
            if arr[i] == 'None':
                arr[i] = 'null'
        importer.import_answer_stat(user, project, arr)
