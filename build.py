...
@app.route("/login_rc_success")
def login_rc_success():
    return login_service.login_rc_success(request)
...
@app.route("/is_rc_chatbot_ready", methods=['POST'])
def is_rc_chatbot_ready(): 
    return chat.is_rc_chatbot_ready(request)
